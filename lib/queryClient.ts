...
export const queryKeys = {
  ...
  studentsByTeacher: (teacherId: string) =>
    ["students", "nested", "byTeacher", teacherId] as const,
  studentsWithoutTeacher: () =>
    ["students", "nested", "withoutTeacher"] as const,
  ...
  studentSubmissions: (studentId: string) =>
    ["student-submissions", studentId] as const,

  // Vocabulary
  vocabulary: (level: string) => ["vocabulary", level] as const,

  // Progress
  flashcardProgress: (userId: string) =>
    ["flashcard-progress", userId] as const,
  studyProgress: (userId: string, days: number) =>
    ["study-progress", userId, days] as const,
...
export const cacheTimes = {
  ...
  flashcardStats: 5 * 60 * 1000, // 5 minutes (reduced from 1 hour)
