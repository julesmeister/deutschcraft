...
export async function getTeacherReview(
  ...
  try {
    const result = await db.execute({
      sql: `
        SELECT tr.*, ws.user_id as student_id 
        FROM teacher_reviews tr
        LEFT JOIN writing_submissions ws ON tr.submission_id = ws.submission_id
    ...
    return {
      ...
      studentId: row.student_id, // From JOIN
      ...
      overallComment: row.comments, // Map to interface property
      ...
      areasForImprovement: row.areas_for_improvement
        ? JSON.parse(row.areas_for_improvement as string)
        : [],
      suggestedEdits: [], // Not currently stored in Turso
    ...
    console.error(
      "[writingService:turso] Error fetching teacher review:",
      error
    );
    throw error;
  }
...
export async function getTeacherReviews(teacherId: string): Promise<any[]> {
  try {
    ...
    console.error(
      "[writingService:turso] Error fetching teacher reviews:",
      error
    );
    throw error;
  }
...
export async function createTeacherReview(reviewData: any): Promise<any> {
  try {
    ...
    console.error(
      "[writingService:turso] Error creating teacher review:",
      error
    );
    throw error;
  }
  ...
  try {
    ...
    if (reviewResult.rows.length > 0) {
      ...
      if (submissionUpdates.length > 0) {
        ...
        await db.batch([
          {
            sql: `UPDATE teacher_reviews SET ${setClauses.join(
              ", "
            )} WHERE review_id = ?`,
            args: args,
          },
          {
            sql: `UPDATE writing_submissions SET ${submissionUpdates.join(
              ", "
            )} WHERE submission_id = ?`,
            args: submissionArgs,
          },
    ...
    console.error(
      "[writingService:turso] Error updating teacher review:",
      error
    );
    throw error;
  }
  ...
  try {
    ...
    const result = await db.execute({
      sql: `
        ...
        FROM teacher_reviews tr
        LEFT JOIN writing_submissions ws ON tr.submission_id = ws.submission_id
        WHERE tr.submission_id IN (${placeholders})
      `,
      args: submissionIds,
    });
    ...
    console.error(
      "[writingService:turso] Error fetching teacher reviews batch:",
      error
    );
    throw error;
  }
