...
export function calculateStreak(progressDocs: StudyProgress[]): number {
  ...
  // Iterate backwards from today to count consecutive days
  // We need to handle the case where "today" might not be in the list yet (user hasn't studied today)
  // but "yesterday" is (streak is alive)
  ...
  // The streak count doesn't include today yet, but is valid.
  ...
  while (true) {
    ...
    if (studyDates.has(dateStr)) {
      ...
      // Safety break to prevent infinite loops (though logical flow prevents it)
