...
export function calculateCourse(
  ...
  const levelData = pricingData[level];
  const intensity = STUDY_INTENSITIES.find((i) => i.hoursPerDay === hoursPerDay) || STUDY_INTENSITIES[1];

  // Calculate adjusted duration
  const adjustedWeeks = Math.ceil(levelData.syllabusWeeks / intensity.multiplier);
...
export function getPricingFeatures(level: CEFRLevel): string[] {
  ...
  if (level === CEFRLevel.B2 || level === CEFRLevel.C1 || level === CEFRLevel.C2) {
