...
import {
  ...
  useIncrementVocabularyUsageMutation,
...
const FloatingRedemittelWidgetBase = ({
  currentLevel,
}: FloatingRedemittelWidgetProps) => {
  const [isOpen, setIsOpen] = useState(false);
  const [isExpanded, setIsExpanded] = useState(false);
  const [activeTab, setActiveTab] = useState<"redemittel" | "saved">(
    "redemittel"
  );
  const [searchTerm, setSearchTerm] = useState("");
  const [selectedLevel, setSelectedLevel] = useState<
    RedemittelCEFRLevel | "all"
  >("all");
  ...
  const { data: savedVocabulary = [], isLoading: savedLoading } =
    useSavedVocabulary(
      session?.user?.email,
...
export const FloatingRedemittelWidget = memo(FloatingRedemittelWidgetBase);
