...
export default function StudentDashboard() {
  ...
  const { data: todayProgressData } = useQuery({
    queryKey: ["todayProgress", session?.user?.email],
    queryFn: async () => {
      if (!session?.user?.email) return { cardsReviewed: 0 };
      return await getTodayProgress(session.user.email);
    },
  ...
  const {
    ...
    refresh: refreshMiniExercise,
  } = useMiniExercise(session?.user?.email || undefined);
  ...
  if (!isFirebaseReady || isLoadingStudent || !session) {
    return (
      <CatLoader message="Loading your dashboard..." size="lg" fullScreen />
    );
  }
