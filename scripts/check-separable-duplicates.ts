/**
 * Check if B1 separable verbs exist in other levels
 */
...
const B1_SEPARABLE_VERBS = path.join(process.cwd(), 'lib/data/vocabulary/split/b1/separable-verbs.json');
...
// Load B1 separable verbs
const b1SeparableData = JSON.parse(fs.readFileSync(B1_SEPARABLE_VERBS, 'utf-8'));
const b1Verbs = b1SeparableData.flashcards.map((c: Flashcard) => c.german.toLowerCase());

console.log('🔍 Checking B1 Separable Verbs for Duplicates\n');
console.log('='.repeat(50));
console.log(`\nB1 Separable Verbs: ${b1Verbs.length} total`);
